/**
⧈ KOBLLUX_Δ³ :: ESPIRITO/beauty/UNIFIED_BEAUTY_SCRIPT.js
#javascript #typescript
Motor de Beleza Textual V3 - Processamento Inline e Observação DOM
Δ7: Gestão de Estado e Componentes (TypeScript)
Opcode 0x0B · ARQUÉTIPO · 528Hz
*/
...
(() => {
        ...
        for (const p of paras) {
            try {
                ...
                if (txt.endsWith('?') && !p.dataset.qProcessed) {
                    ...
                    wrap.className = 'q-card';                    wrap.innerHTML = `<div class="q-ico">?</div><div class="q-body">${esc(txt)}</div>`;
                    p.replaceWith(wrap);
                }
            } catch (err) {
                /* silent */
        ...
        $$('p', container).forEach(p => {
            try {
                ...
                if (t.length > 600 && t.includes('. ')) {
                    ...
                    if (mark > 0) {
                        const a = t.slice(0, mark + 1);
                        const b = t.slice(mark + 1);
                        const p2 = p.cloneNode();
                        p2.textContent = b.trim();
    ...
    const asciiScore = (t = '') => {
        ...
        let h = 0;
        for (const ln of L) {
            if (box.test(ln) || grid.test(ln) || ln.trim().startsWith('> ')) h++;
        }
    ...
    const enhanceASCII = (root = document) => {
        ...
        for (const el of cand) {
            ...
            if (!asciiScore(txt)) continue;
            const fig = document.createElement('figure');
            fig.className = 'ascii-card';
            const pre = document.createElement('pre');
            pre.textContent = txt;            fig.appendChild(pre);
            if (!el.closest('pre')) {
                const fc = document.createElement('figcaption');
                fc.className = 'ascii-cap';
                fc.textContent = 'ASCII • renderizado em bloco';
                fig.appendChild(fc);
            }
            el.replaceWith(fig);
        }
    ...
    document.addEventListener('click', e => {
        ...
        if (chip) {
            ...
            const ev = new CustomEvent('chip:click', {
                detail: {
                    label,
                    source: 'unified-beauty'
                }
            });
