...
(function () {
  ...
  const GLOBAL_STATE_KEY = 'dual_windows_state';
  ...
  /* =========================================================
     SESSION LIFECYCLE · SMART ACTIVITY
     ========================================================= */
  ...
  function renderTabSwitcher(win) {
    ...
    data.tabs.forEach(tab => {
      ...
      // --- NOVO: duplo clique para editar URL ---
      ...
      titleEl.addEventListener('dblclick', function(e) {
        ...
        input.addEventListener('blur', function() {
          ...
          if (newUrl) {
            updateTabUrl(win, tab.id, newUrl);
          }
          const newTitle = newUrl ? newUrl.replace(/^https?:\/\//, '').split('/')[0] : 'Nova Aba';
  ...
  function saveGlobalState() {
    ...
    document.querySelectorAll('.session-window').forEach(win => {
      ...
      windows.push({
        ...
        peeked: win.classList.contains('peeked')
      });
    });
    try {
      localStorage.setItem(GLOBAL_STATE_KEY, JSON.stringify(windows));
    } catch (_) {}
  }

  function createSessionWindowFromState(state) {
    const id = state.id || 'session-' + Date.now() + '-' + counter++;
    ...
    if (state.collapsed) win.classList.add('collapsed');
    if (state.peeked) win.classList.add('peeked');
    win.id = id;
    win.innerHTML = `
      <div class="win-hdr">
        ...
        <div class="win-navrow" style="flex:2;min-width:0;pointer-events:auto;">
          <input class="win-urlbar" type="text" value="" placeholder="Digite uma URL..." spellcheck="false" autocomplete="off">
    ...
    const activeId = state.activeId || (tabs[0]?.id);
    if (tabs.length) {
      tabDataMap.set(win, { tabs, activeId });
    } else {
      const fallback = [{ id: 'tab-'+Date.now(), url: 'https://www.infodose.com.br/splash', title: 'Nova Aba', fav: false, icon: '◉' }];
      tabDataMap.set(win, { tabs: fallback, activeId: fallback[0].id });
    }
    renderTabCounter(win);
    ...
    frame.src = activeTab ? activeTab.url : 'https://www.infodose.com.br/splash';
    win.appendChild(frame);
    
    // Conectar eventos
    wireSession(win);
    
    // Se minimizada, adicionar ao dock
    if (state.minimized) {
  ...
  function restoreGlobalState() {
    ...
    try {
      ...
      if (windows.length) {
        windows.forEach(state => createSessionWindowFromState(state));
        return true;
      }
    } catch (_) {}
    return false;
  ...
  /* ---------------------------------------------------------
     wireSession (com persistência extra)
     --------------------------------------------------------- */
  ...
  // Inicialização com restauração global
  const restored = restoreGlobalState();
  if (!restored) {
    createSessionWindow();
  }
  // Se não houver nenhuma janela (caso o estado salvo estivesse vazio), criar uma
  if (!document.querySelector('.session-window')) {
    createSessionWindow();
  }
  ...
  window.saveGlobalState = saveGlobalState; // expor para debug

  console.log('🚀 Almasliber OS — Core com Persistência Global e Edição de Abas carregado.');
})();