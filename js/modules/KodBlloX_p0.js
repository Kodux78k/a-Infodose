...
const dock =
  document.querySelector(
    `.dock-bubble[data-session-id="${CSS.escape(String(sid))}"]`
  );
if (dock) {
  try {
    dock.remove();
...
/* loose mirror */
$$(
  `[data-dock-mirror="1"][data-session-id="${CSS.escape(String(sid))}"]`
).forEach((mirror) => {
  try {
    mirror.remove();
  } catch (_) {}
...
let bubble =
  dock.querySelector(
    `.dock-bubble[data-session-id="${CSS.escape(String(id))}"]`
  );
...
dock.addEventListener(
  ...
  (event) => {
    ...
    if (!bubble || !dock.contains(bubble)) {
      return;
    }
...
if (!loose || loose.__KBLX_P_BOUND__) {
  return;
}
  ...
  SL.evict =
    function (id) {
      ...
      const lastSuspended =
        ...
        Date.now();
      const age =
        Date.now() - lastSuspended;
      if (age < HARD_SUSPEND_MS) {
        return false;
      }
...
if (
document.body.KBLX_P_SRCDOC_GUARD
) {
return;
}
