(function() {
  ...
  // =========================================
  // 2.1 NÚCLEO Δ-STEP 78K - INTEGRADO
  // =========================================
  function sigmoid(x) {
  ...
  /**
   * Atualiza estado 78K com limite Δ
   * @param {number} prev - Estado anterior 0-1
   * @param {number} v - Verdade do sinal 0-1
   * @param {number} i - Integrar 0-1
   * @param {number} deltaStep - Velocidade máxima por ciclo
   * @returns {number} Novo estado 0-1
   */
  function update78K(prev, v, i, deltaStep = 0.05) {
    const eps = 1e-6;
    const energy = v * i;
  ...
  ARCHETYPES.forEach(id => {
    if (typeof archWeights[id]!== 'number') archWeights[id] = 0.1; // inicia baixo
  });
  ...
  // =========================================
  // 5. FUNÇÃO GERAR FRACTAIS (COM MOTOR + Δ-STEP)
  // =========================================
  ...
  function generateFractals() {
    ...
    sentences.forEach((sentence, i) => {
      ...
      // =========================================
      // Δ-STEP 78K: Atualiza peso do arquétipo com limite
      // v = 1.0 se arquétipo está na sequência, senão 0.3
      // i = 0.8 força de integração padrão
      // deltaStep = 0.05 = 5% por ciclo
      // =========================================
      const v = sequence.includes(currentArchName)? 1.0 : 0.3;
      const prevWeight = archWeights[currentArchName] || 0.1;
      archWeights[currentArchName] = update78K(prevWeight, v, 0.8, 0.05);
      ...
      block.style.setProperty('--weight-opacity', weight); // usa peso pra opacidade/sombra
    ...
    if (dom.statusBar) dom.statusBar.textContent = `Opcode 0x0B · Motor 3·6·9 · ${total} Fractal(s) · Δ78K ON`;
    ...
    showToast(`Integração concluída | Motor: +${di_engineStep} | Reverse: ${di_reverse? 'ON' : 'OFF'} | Salto: +${di_jump} | ${di_use3697? 'Ciclo 3697' : 'Linear'} | Δ78K`);
  }
