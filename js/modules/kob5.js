...
(() => {
  ...
  try{
    ...
    __bridgeSpeaking = false;
  }

  break;
        
      case "LOG":
        console.log("[kob5]", msg.payload);
        break;
    }
  });
...
let __bridgeSpeaking = false;

if(window.KOBLLUX.speakText){
  const oldSpeak = window.KOBLLUX.speakText;

  window.KOBLLUX.speakText = function(text, opts){

    if(!__bridgeSpeaking){
      window.KOBLLUX.send("SPEAK", {
        text
      });
    }
