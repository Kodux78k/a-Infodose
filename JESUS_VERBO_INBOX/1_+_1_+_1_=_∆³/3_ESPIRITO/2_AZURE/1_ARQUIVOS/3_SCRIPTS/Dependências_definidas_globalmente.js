(function(global) {
  ...
  const DB = global.KOBLLUX_PLAYER_DB || global.KOBLLUX_DB; 
// fallback para compatibilidade
  ...
  let state = {
    ...
    ytPlayer: null,
    scWidget: null,
    ytReady: false,        // agora indica que o player está pronto, não a API
    widgetState: "ball",
    isDragging: false,
  ...
  function syncIcons() {
    ...
    if (dom.prevPlayIcon) dom.prevPlayIcon.className = `ph-fill ${icon} text-4xl`;
    if (dom.footPlayIcon) dom.footPlayIcon.className = `ph-fill ${icon} text-5xl`;
    if (dom.mainPlayIcon) dom.mainPlayIcon.className = `ph-fill ${iconSimple} text-3xl ${state.isPlaying ? "" : "ml-1"}`;
  }
  ...
  function renderTabs() {
    ...
    ordered.forEach(pl => {
      ...
      const icon = pl.id === ALL_ID ? "ph-stack" :
                   pl.id === FAVORITES_ID ? "ph-heart" :
                   isCadial ? "ph-spiral" : "ph-playlist";
  ...
  function renderPlaylist() {
    ...
    visible.forEach(t => {
      ...
      item.innerHTML = `
        ...
        ${activeItem && state.isPlaying ? '<i class="ph-fill ph-waveform text-[var(--primary)] animate-pulse ml-1"></i>' : ""}
      `;
  ...
  function ensureYTPlayer() {
    if (state.ytPlayer) {
      // Verifica se o player já existe e está funcional
      if (state.ytReady) {
    ...
    // Cria o player
    state.ytReady = false; // ainda não pronto
    state.ytPlayer = new YT.Player(dom.ytContainer, {
      ...
      events: {
        onReady: (e) => {
          ...
          state.ytReady = true;
          // Se já havia uma track pendente, carrega
  ...
  function playYT(track) {
    // Se não há track, não faz nada
    if (!track) return;

    // Se o player não está pronto, aguarda
    if (!state.ytReady) {
      // Armazena a track pendente
      state.pendingTrackId = track.id;
      // Se o player ainda não foi criado, cria
      if (!state.ytPlayer) {
        ensureYTPlayer();
      }
      return;
...
function playSC(url) {
  ...
  // Cria o iframe com a URL (pode ser encurtada, o SoundCloud resolve internamente)
  ...
  state.scWidget.bind(SC.Widget.Events.ERROR, (e) => {
    ...
    // Fallback: recarregar o iframe após 1s
    setTimeout(() => {
      if (dom.scContainer) {
        dom.scContainer.innerHTML = '';
        playSC(url); // tenta novamente
      }
    }, 1000);
  });
}
 ...
 function loadAndPlayById(trackId) {
  ...
  try { 
    if (state.scWidget) {
      ...
      // Limpa o container para evitar widgets fantasmas
  ...
  syncPreviewAndMain(track);
    ...
    if (track.type === "youtube" || track.type === "youtube_playlist") {
      ...
      ensureYTPlayer();
      // Inicia a reprodução (pode ser síncrono ou assíncrono)
      playYT(track);
  ...
  function initKoduxPlayer() {
    ...
    // Expõe funções globais (para os atributos onclick do HTML)
    ...
    // Quando a API do YouTube estiver carregada, a inicialização do player será feita sob demanda
    global.onYouTubeIframeAPIReady = function() {
      console.log("YouTube API ready");
      // Não definimos state.ytReady aqui, apenas criamos o player quando necessário
      // O player será criado na primeira chamada de ensureYTPlayer()
    };

    // Carrega API do YouTube se não estiver presente
    if (typeof YT === 'undefined' || typeof YT.Player === 'undefined') {
      ...
      // Se já estiver disponível, define a função
