(function () {
  ...
  const sessionStore   = new Map();        // snapshots de sessões suspensas
  const suspendedQueue = [];               // FIFO para limite de suspensas
  const sessionMeta    = new Map();        // metadados de cada sessão
  const tabDataMap     = new WeakMap();    // dados de abas por janela (DOM)

  let counter      = 1;
  let activeWindow = null;                // referência à janela ativa
  ...
  // ═══════════════════════════════════════════════════════════════════
  // 11. AUDITORIA E PULSO DO SISTEMA
  // ═══════════════════════════════════════════════════════════════════

  window.DualSessionAudit = function() {
    const active = SessionLifecycle.listSessions().filter(s => !/closed|suspended/.test(s.state));
    return {
      slotsVivos: active.length,
      slotsOciosos: active.filter(s => s.state === 'idle').length,
      proximoASuspensao: active.filter(s => Date.now() - (s.lastActive || 0) > 80_000).map(s => s.id)
    };
  };
  ...
  console.log('🚀 Almasliber OS — Unified Core com Abas, Snapshot e Auditoria carregado.');
})();