    ...
    function makeOrbAvatar(name = 'DUAL', size = 64) {
      ...
      return `
        <div class="dual-orb-wrap" style="--orb-size:${size}px; --orb-primary:hsl(${h1},100%,62%); --orb-secondary:hsl(${h2},92%,48%);" aria-label="${safe}" role="img">
          <svg class="dual-orb-svg" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <defs>
              <radialGradient id="orb_core_${seed}" cx="50%" cy="50%" r="50%">
              ...
              <linearGradient id="orb_ring_${seed}" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stop-color="hsl(${h1},100%,76%)"/>
                <stop offset="100%" stop-color="hsl(${h2},100%,58%)"/>
              </linearGradient>
            </defs>
            <circle cx="50" cy="50" r="46" fill="#05070c"/>
            <circle cx="50" cy="50" r="40" fill="url(#orb_core_${seed})" opacity=".28"/>
            <circle cx="50" cy="50" r="38" fill="none" stroke="url(#orb_ring_${seed})" stroke-width="1"/>
            <circle cx="50" cy="50" r="46" fill="none" stroke="url(#orb_ring_${seed})" stroke-width="2.5" stroke-dasharray="70 20 10 30" stroke-linecap="round" opacity=".86"/>
