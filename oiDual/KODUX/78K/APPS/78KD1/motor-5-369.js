(function() {
  ...
  // FONTE ÚNICA: ARCHETYPES / ARCH_NAMES / ARCH_COLORS são "let"
  // compartilhados por todo o script. O fetch remoto (mais abaixo)
  // ATUALIZA essas mesmas variáveis em vez de criar uma 2ª fonte.
  // =========================================
  ...
  const ARCH_NAMES_BASE = {
    ...
    elysha: "Elysha",
    luxara: "Luxara", // FIX: era "Luxara" (maiúsculo) e nunca batia com o id "luxara"
    velor: "Velor",
    ...
    aek_dion: "a€K_Dion",             // FIX: era "k_dion", não batia com o id "aek_dion"
    aekael_domnnus: "a€Kael DommnuS", // FIX: era "kael_domnnus", não batia com "aekael_domnnus"
    nephesh_elyon: "a€Nephesh Elyon"
  };
  if (userKey) ARCH_NAMES_BASE[userKey] = di_userNameRaw.trim();

  // fonte única e MUTÁVEL — motor, select e color probe leem sempre destas 3 variáveis
  ...
  function publishGlobals() {
    ...
    // compatibilidade com outros arquivos do ecossistema que já liam estes globais
  ...
  publishGlobals();
  populateArchSelect(); // 1ª renderização, síncrona, com a lista local — nunca fica inerte
  ...
  // Carrega a lista remota em background e, se vier ok, SUBSTITUI
  // as mesmas ARCHETYPES/ARCH_NAMES/ARCH_COLORS (sem criar uma 2ª fonte).
  (async function loadRemoteArchetypes() {
    ...
    try {
      ...
      ARCH_COLORS = colors;
      publishGlobals();
      populateArchSelect(); // re-renderiza com a lista remota, preservando seleção se possível
      ...
      // ARCHETYPES/ARCH_NAMES continuam com os valores locais já publicados acima
    }
  })();
  ...
  // probe único e reaproveitado (em vez de criar/destruir uma <body> por frase)
  ...
  function getArchColor(archId) {
    // prioridade 1: cor vinda do archetypes.json (sem custo de layout)
    if (ARCH_COLORS[archId]) return ARCH_COLORS[archId];
    // prioridade 2: fallback via CSS custom property.
    // ATENÇÃO: isso assume um seletor CSS genérico "[data-arch=...]".
    // Se o CSS só define a cor em "body[data-arch=...]", troque para usar dom.body aqui.
