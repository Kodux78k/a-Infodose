...
function createPreview(item) {
    ...
    if (type === "html") {
        return `<div class="file-preview"><span class="type-badge">HTML</span><div class="preview-text">${escapeHTML(previewFromHTML(item.content))}</div></div>`;
    }
...
function renderHero() {
    ...
    items.forEach((item, index) => {
        ...
        card.querySelector(".hero-listen").addEventListener("click", (e) => {
            e.stopPropagation();
            // Tenta usar o Player primeiro, se disponível
            if (window.NebulaPlayer && typeof window.NebulaPlayer.loadExternalItem === 'function') {
                window.NebulaPlayer.loadExternalItem(item);
                if (typeof window.NebulaPlayer.togglePlay === 'function') {
                    window.NebulaPlayer.togglePlay();
...
(function(){
  ...
  // Overlay viewer (pode não existir, então usamos placeholders)
  ...
  const NebulaPlayer = {
    // Carrega um item externo (vindo do Unified)
    loadExternalItem: async function(item) {
      if (!item) return;
      // Tenta obter o conteúdo/url
      let content = item.content || '';
      let url = item.url || '';
      let type = item.type || 'txt';
      let name = item.name || 'Documento';

      // Se for PDF ou HTML e tiver URL, tenta carregar via pdfjs
      if ((type === 'pdf' || type === 'html') && url) {
        try {
          ...
          // Fallback para conteúdo textual
        ...
        // Para Markdown, JSON, TXT
        let text = content || '';
        // Se for JSON, tenta pretty print
        if (type === 'json') {
...
(function(){
  ...
  // Fecha o DEV PANEL ao clicar fora
...
(function(){
  ...
  group('📦 MÓDULOS DETECTADOS', () => {
    modules.forEach(mod => {
      ...
      if (loaded) {
        ...
        // Lista propriedades adicionais não-função (para inspeção)
  ...
  group('⚙️ FUNCIONALIDADES PRINCIPAIS', () => {
    ...
    c.log('• Player tem suporte a OCR, destaque de palavras, timeline, bookmarks, busca.');
  });
