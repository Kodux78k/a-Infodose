...
<title>Kodux Player + Visualizer</title>
...
<style>
    ...
    /* ===== VISUALIZER OVERLAY (agora com player de verdade) ===== */
    #visualizer-overlay {
        position: fixed;
        top: 0;
        left: 0;
    ...
    /* Container que abriga o player (YT ou SC) */
    #visualizer-overlay .player-box {
        width: 90%;
        max-width: 800px;
        height: 70vh;
        max-height: 600px;
        ...
        position: relative;
        border: 1px solid rgba(255,255,255,0.1);
        transition: width 0.3s ease, height 0.3s ease, border-radius 0.3s ease, max-width 0.3s ease;
    }
    #visualizer-overlay .player-box iframe {
        width: 100%;
        height: 100%;
        border: none;
        display: block;
    }
    /* Info da faixa dentro do overlay */
    #visualizer-overlay .viz-info {
        position: absolute;
        bottom: 20px;
        left: 20px;
    ...
    /* Barra de controles: minimizar / telão / fechar (padrão Global Player) */
    #visualizer-overlay .viz-toolbar {
        position: absolute;
        top: 16px;
        right: 16px;
        display: flex;
    ...
    #visualizer-overlay .viz-btn:hover {
        background: rgba(255,255,255,0.2);
    }
    #visualizer-overlay .viz-btn.active {
        background: #00ffcc;
        color: #000;
        border-color: #00ffcc;
    }
    #visualizer-overlay .viz-btn-close:hover {
        background: rgba(255,0,0,0.3);
        border-color: #ff4444;
    }
    /* Modo telão: esconde info e fundo escuro */
    #visualizer-overlay.telao .viz-info {
        opacity: 0;
        pointer-events: none;
        transition: opacity 0.3s;
    }
    #visualizer-overlay.telao .player-box {
        border-radius: 0;
        width: 100vw;
        height: 100vh;
        max-width: none;
    ...
    /* ===== MODO MINIMIZADO (estilo Global Player: só um cartãozinho no
       canto, tocando em segundo plano, toque pra reabrir) ===== */
    #visualizer-overlay.minimized {
    ...
    #visualizer-overlay.minimized .viz-info,
    #visualizer-overlay.minimized .viz-btn-telao {
        display: none;
    }
    #visualizer-overlay.minimized .player-box {
        width: 160px;
        height: 100px;
        max-width: 160px;
        max-height: 100px;
    ...
    #visualizer-overlay.minimized .viz-toolbar {
        pointer-events: auto;
        top: auto;
        bottom: calc(100px + 6px);
        right: 16px;
    }
    #visualizer-overlay.minimized .viz-btn-min {
        background: #00ffcc;
        color: #000;
        border-color: #00ffcc;
    }
    @media (max-width: 600px) {
        #visualizer-overlay .viz-btn { width: 34px; height: 34px; font-size: 14px; }
        #visualizer-overlay .viz-info { bottom: 12px; left: 12px; font-size: 11px; padding: 4px 12px; }
        #visualizer-overlay .viz-toolbar { top: 10px; right: 10px; }
        #visualizer-overlay .player-box { height: 60vh; }
    }

    /* ===== ANIMAÇÃO LEVE NO WIDGET PRINCIPAL ===== */
    /* Quando está tocando, uma borda com gradiente suave (sem glow pesado) */
    ...
    @keyframes widgetPulse {
        0%   { box-shadow: 0 0 0 1px rgba(0, 210, 255, 0.1); }
        50%  { box-shadow: 0 0 0 3px rgba(0, 210, 255, 0.25); }
        100% { box-shadow: 0 0 0 1px rgba(0, 210, 255, 0.1); }
    }

    /* Botão de visualizador desativado (faixa local não usa mais visualizer) */
    .btn-visualizer.viz-disabled {
        opacity: 0.3;
        pointer-events: none;
    }

    /* Os containers YT/SC agora ficam dentro do overlay, não mais escondidos */
    ...
    /* Quando o overlay está aberto, mostramos o container ativo */
    #visualizer-overlay.open #yt-container.active,
    #visualizer-overlay.open #sc-container.active {
...
<div id="bodyPlayer" data-mode="player">
    ...
    <div id="kodux-widget" data-idle-target="" class="state-ball" style="position: absolute; right: 20px; bottom: 100px;">
      ...
      <div id="content-preview" class="hidden-content drag-header">
        ...
        <!-- Botão visualizador no preview -->
        <button onclick="toggleVisualizer(event)" class="btn-ctrl btn-visualizer transition-base" style="margin-left:4px;" title="Visualizador">
      ...
      <div id="content-footer" class="hidden-content">
        ...
        <div class="footer-drag-header drag-header">
          ...
          <div class="controls-footer">
            ...
            <!-- BOTÃO DO VISUALIZADOR (footer) -->
            <button onclick="toggleVisualizer(event)" class="btn-ctrl btn-visualizer transition-base" title="Visualizador">
              <svg class="kx-icon" aria-hidden="true" viewBox="0 0 24 24" fill="currentColor" width="1em" height="1em">
                <use href="#kx-waveform"></use>
              </svg>
            </button>
      ...
      <div id="content-full" class="hidden-content">
        <div class="full-header drag-header">
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            <button onclick="toggleVisualizer(event)" class="btn-ctrl btn-visualizer transition-base" title="Visualizador" style="background:rgba(255,255,255,0.05);border-radius:50%;padding:6px;">
...
<div id="visualizer-overlay">
    <div class="viz-info" id="viz-track-info">Nenhuma faixa</div>
    <div class="session-window player-box" id="player-box" onclick="handlePlayerBoxClick(event)">
        <!-- Container do YouTube -->
        <div id="yt-container"></div>
        <!-- Container do SoundCloud -->
        <div id="sc-container"></div>
    </div>
    <div class="viz-toolbar">
        <button id="viz-min-btn" class="viz-btn viz-btn-min" onclick="toggleMinimize()" title="Minimizar">–</button>
        <button id="viz-telao-btn" class="viz-btn viz-btn-telao" onclick="toggleTelao()" title="Telão (tela cheia)">⛶</button>
        <button class="viz-btn viz-btn-close" onclick="toggleVisualizer()" title="Fechar visualizador">✕</button>
    </div>
</div>

<!-- SPRITE DE ÍCONES (completo) -->
...
// ------- IndexedDB: armazenamento dos blobs de áudio importados -------
const KoduxAudioIDB = (() => {
...
// =========================================================================
// PLAYER + VISUALIZER (unificado, sem canvas)
// =========================================================================
(function(global) {
  "use strict";
  ...
  function initDOM() {
    ...
    dom.vizOverlay = document.getElementById("visualizer-overlay");
    dom.vizInfo = document.getElementById("viz-track-info");
  }
  ...
  // ===== VISUALIZADOR (player real: vídeo/widget, sem canvas) =====
  function updateVizInfo(track) {
    if (!dom.vizInfo) return;
    if (track) {
      dom.vizInfo.textContent = \`\${track.name} — \${track.artist}\`;
    } else {
      dom.vizInfo.textContent = 'Nenhuma faixa';
    }
  }

  // Só faz sentido pra YouTube/SoundCloud — áudio local não usa visualizer.
  function trackSupportsVisualizer(track) {
    return !!(track && (track.type === 'youtube' || track.type === 'youtube_playlist' || track.type === 'soundcloud'));
  }

  function refreshVisualizerAvailability() {
    const track = state.currentTrackId ? getTrackById(state.currentTrackId) : null;
    const enabled = trackSupportsVisualizer(track);
    document.querySelectorAll('.btn-visualizer').forEach(btn => {
      btn.classList.toggle('viz-disabled', !enabled);
      btn.title = enabled ? 'Visualizador' : 'Visualizador indisponível para áudio local';
    });
  }

  function showPlayerInOverlay(track) {
    if (!dom.ytContainer || !dom.scContainer) return;
    dom.ytContainer.classList.remove('active');
    dom.scContainer.classList.remove('active');
    if (!track) {
      updateVizInfo(null);
      return;
    }
    ...
    if (track.type === 'youtube' || track.type === 'youtube_playlist') {
      dom.ytContainer.classList.add('active');
      if (!state.ytPlayer) ensureYTPlayer();
    } else if (track.type === 'soundcloud') {
      dom.scContainer.classList.add('active');
    }
  }

  function toggleVisualizer(e) {
    if (e) e.stopPropagation();
    const overlay = dom.vizOverlay;
    if (!overlay) return;
    const track = state.currentTrackId ? getTrackById(state.currentTrackId) : null;

    // Áudio local não tem vídeo/widget pra mostrar — não abre nada.
    if (!overlay.classList.contains('open') && !trackSupportsVisualizer(track)) {
      return;
    }

    const isOpen = overlay.classList.contains('open');
    if (isOpen) {
      overlay.classList.remove('open', 'telao', 'minimized');
      document.getElementById('viz-telao-btn')?.classList.remove('active');
      document.getElementById('viz-min-btn')?.classList.remove('active');
    } else {
      overlay.classList.remove('minimized');
      overlay.classList.add('open');
      showPlayerInOverlay(track);
      if ((track.type === 'youtube' || track.type === 'youtube_playlist') && !state.ytReady) {
        ensureYTPlayer();
      }
      if (track.type === 'soundcloud' && !state.scWidget) {
        playSC(track.url);
      }
    }
  }

  // Toque no cartãozinho minimizado reabre o overlay em tamanho normal.
  ...
  // Minimizar: vira um cartãozinho pequeno no canto (estilo Global Player),
  // continua tocando, um toque nele reabre. Não é a mesma coisa que fechar.
  function toggleMinimize() {
    const overlay = dom.vizOverlay;
    if (!overlay || !overlay.classList.contains('open')) return;
    overlay.classList.remove('telao');
    document.getElementById('viz-telao-btn')?.classList.remove('active');
    const nowMinimized = overlay.classList.toggle('minimized');
    document.getElementById('viz-min-btn')?.classList.toggle('active', nowMinimized);
  }

  function toggleTelao() {
    const overlay = dom.vizOverlay;
    if (!overlay) return;
    overlay.classList.remove('minimized');
    document.getElementById('viz-min-btn')?.classList.remove('active');
    overlay.classList.toggle('telao');
    const btn = document.getElementById('viz-telao-btn');
    if (btn) btn.classList.toggle('active', overlay.classList.contains('telao'));
  }
  ...
  function syncPreviewAndMain(track) {
    ...
    if (dom.vizOverlay?.classList.contains('open')) {
      showPlayerInOverlay(track || null);
    }
    refreshVisualizerAvailability();
  }
  ...
  function loadAndPlayById(trackId) {
    ...
    // Se o visualizer estava aberto numa faixa local (não devia, mas por
    // segurança) e a nova faixa também não suporta, fecha o overlay.
    if (dom.vizOverlay?.classList.contains('open') && !trackSupportsVisualizer(track)) {
      dom.vizOverlay.classList.remove('open', 'telao', 'minimized');
    }
  ...
  function initKoduxPlayer() {
    ...
    global.toggleVisualizer = toggleVisualizer;
    global.toggleTelao = toggleTelao;
    global.toggleMinimize = toggleMinimize;
    global.handlePlayerBoxClick = handlePlayerBoxClick;
    ...
    // Duplo clique na bola abre o visualizador (só funciona se a faixa
    // atual for YouTube/SoundCloud — trackSupportsVisualizer cuida disso)
    ...
    console.log("⚫ KODUX Player + Visualizer inicializado (modo leve).");
  }
